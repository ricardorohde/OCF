   ...
   $sql = sprintf("select i.campeonato,i.userid ".
        ...
        if ($db->NumRows() == 0) {
        	echo ('         <tr> <td>Não existe campeonatos cadastrados ou em andamento no momento.<br></td> </tr>')."\n";
		         ...
		         while ($db->Next()) {
		                if ($camp <> $db->getValue('campeonato')) {
						    ...
						    $camp = $cmp->getCodigo();
		                    echo ("<tr  class='cabec'><td align=center>Pos</td><td>Participante</td><td align=center>Pts</td><td align=center>&nbspB&nbsp</td></tr>\n");                	
		                }
			...
			/*				"<td align=center style='".(($row['flag1'] == 'S') ? 'background:rgb(255, 194, 133);' : 'background:rgb(255, 245, 210);')."'>".$acr1.(($row['qtdb1'] == 0) ? "&nbsp&nbsp&nbsp&nbsp" : $row['qtdb1'])."</td>".
							...
							( (($ins->getBonus() + $ins->getBonusRecRodadas()) == 0) ? "&nbsp&nbsp" : ($ins->getBonus() + $ins->getBonusRecRodadas()))."</td>"."</tr>"."\n";
 		                echo ($linha);                	
						$usr = NULL;
						$ins = NULL;
		         }
				echo("<tr><td colspan=10><br></td></tr>")."\n";
		        echo("<tr><td colspan=10 rowspan=2>(*) Bonus recorde rodada</td></tr>")."\n"; 
        }
