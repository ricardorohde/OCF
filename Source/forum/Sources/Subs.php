...
{
	global $modSettings;
    echo ("<h5> Host_from_ip 1 </h5>");
	if (($host = cache_get_data('hostlookup-' . $ip, 600)) !== null)
		return $host;

    echo ("<h5> Host_from_ip 2 </h5>");

	$t = microtime();
    echo ("<h5> Host_from_ip 3 </h5>");

	// If we can't access nslookup/host, PHP 4.1.x might just crash.
	if (@version_compare(PHP_VERSION, '4.2.0') == -1)
		$host = false;
    echo ("<h5> Host_from_ip 4 </h5>");

    echo ("<h5>".$ip."</h5>");
    echo ("<h5>".@escapeshellarg($ip)."</h5>");
    ...
    echo ("<h5> Host_from_ip 6 </h5>");

	// This is the last try :/.
	if (!isset($host) || $host === false)
		$host = @gethostbyaddr($ip);
    echo ("<h5> Host_from_ip 7 </h5>");

	// It took a long time, so let's cache it!
	if (array_sum(explode(' ', microtime())) - array_sum(explode(' ', $t)) > 0.5)
		cache_put_data('hostlookup-' . $ip, $host, 600);
    echo ("<h5> Host_from_ip 8 </h5>");
