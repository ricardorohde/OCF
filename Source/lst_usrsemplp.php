...
<?php 
  ...
  $dw = array("Dom","Seg","Ter","Qua","Qui","Sex","Sab"); 
	...
	include ('conectadb.php');
        ...
        $verifica = sprintf("select c.codigo codcamp, c.ano,r.rodada, c.descricao,
		min(addtime(data, hora)) datahora
       ...
       if (mysql_num_rows($result) == 0) {
      	   echo("<tr><td colspan=7>Nenhuma rodada cadastrada no momento.</td></tr>");
               ...
               $descr = sprintf('<a href="frm_usrsemplp.php?camp=%s&rod=%s">%s-%s</a>',$row['codcamp'],
					$row['rodada'],$row['descricao'],$row['ano']);
               $rodada = sprintf('<a href="frm_usrsemplp.php?camp=%s&rod=%s">%02d</a>',$row['codcamp'],
					$row['rodada'],$row['rodada']);
               $dh = sprintf('<a href="frm_usrsemplp.php?camp=%s&rod=%s">%s %s <i>%s</i></a>',$row['codcamp'],
